...
Irssi::settings_add_str('notify', 'notify_wmclass', 'Irssi');
...
sub notify {
    ...
    my $setclass = Irssi::settings_get_str('notify_wmclass');

    # wiggle the launcher icon 
    my $cmd = "EXEC - wmctrl -xr '" . $setclass . "' -b add,demands_attention";
    $server->command($cmd);

    # don't notify if Irssi is active
    $cmd = <<'END';
xprop -id\
